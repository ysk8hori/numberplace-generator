...
export function createGame(
  { blockSize, difficulty, gameType }: GameInfo,
): { status: 'success'; game: Game } | { status: 'error' } {
  ...
  while (true) {
    試行回数++;
    if (35 < 最大試行回数を取得する(blockSize, gameType)) { // history を巻き戻して試すよりも、ダメなら最初からやり直すほうが早いようだ。
      // 試行回数が多すぎる場合は問題を生成できないとみなす
      return { status: 'error' };
    }
    let flg = false;
    // if (試行回数 % 10 === 0) {
    //   // 80回以上の試行回数で、10回ごとにスナップショットを取り、前回と同じスナップショットが取れた場合は無限ループに陥っているため巻き戻しを行う。 → 試行回数が多い場合は巻き戻すよりも最初からやり直すほうが早いようなので不要になった
