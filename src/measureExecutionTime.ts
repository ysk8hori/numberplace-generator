...
function saveToCsv(
  ...
  try {
    ...
    if (!existsSync(csvPath)) {
      const header = 'timestamp\tbranch\tnode_version\tstandard_avg_ms\tcross_avg_ms\thyper_avg_ms\n';
      writeFileSync(csvPath, header, 'utf8');
    }

    // データ行を追記
    const row = `${timestamp}\t${branch}\t${nodeVersion}\t${averages.get('standard')}\t${averages.get('cross')}\t${averages.get('hyper')}\n`;
...
async function measureExecutionTime() {
  ...
  gameTypes.forEach(gameType => {
    ...
    console.log(`${gameType}: ${avg.toFixed(2)}`);
  });
